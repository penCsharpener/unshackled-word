...
public class GithubFileDownloader : IFileDownloader
{
    ...
    public GithubFileDownloader(IFileService fileService, HttpClient httpclient, IOptions<AppSettings> options, ILogger<GithubFileDownloader> logger)
    {
    ...
    public async Task DownloadFileAsync(CancellationToken token = default)
    {
        ...
        foreach (var file in _options.DatabaseSeeding.SRFileUrls)
        {
            ...
            var filePath = _fileService.Combine(_options.DatabaseSeeding.FolderLocation, "SR", fileName.Replace("%20", " "));
