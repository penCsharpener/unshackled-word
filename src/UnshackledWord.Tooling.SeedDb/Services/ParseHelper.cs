...
{
    public BibleBook ParseNtBookId(string verseString)
    {
    ...
    public BibleReference ParseNtVerseId(string verseString)
    {
        if (int.TryParse(verseString, out var verseId) is false)
        {
            throw new ArgumentException("Verse Id is not a number.", nameof(verseString));
        ...
        return new BibleReference { BookId = bookId, Chapter = chapterId, Verse = verseIdInChapter };
    }
}
