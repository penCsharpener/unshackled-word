...
public class BibelKommentareDownloader : IFileDownloader
{
    ...
    public BibelKommentareDownloader(IFileService fileService, HttpClient httpclient, IOptions<AppSettings> options, ILogger<GithubFileDownloader> logger)
    {
    ...
    public async Task<List<string>> DownloadFileAsync(CancellationToken token = default)
    {
        ...
        var response = await _httpclient.GetAsync(_options.DatabaseSeeding.BibleKommentareElberfelderUrl, HttpCompletionOption.ResponseHeadersRead, token);
